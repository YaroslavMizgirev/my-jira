...
class ActionTypeTest {
    ...
    void testNameUniqueness() {
        ...
        // Проверка что разные ID но одинаковое имя считаются равными
        assertNotEquals(actionType1.getId(), actionType2.getId());
        assertEquals(actionType1.getName(), actionType2.getName());
    }

    @Test
    @DisplayName("Should trim name when setting")
    void testSetNameTrimsValue() {;
        assertEquals("STATUS_CHANGE", ActionType.builder().name("  STATUS_CHANGE  ").build().getName());

        ActionType type = new ActionType();
        type.setName("    STATUS_CHANGE      ");
        assertEquals("STATUS_CHANGE", type.getName());
    }

    @Test
    @DisplayName("Should throw exception when setting null name")
    void testSetNameWithNull() {
        assertThrows(IllegalArgumentException.class, () -> ActionType.builder().name(null).build());
        assertThrows(IllegalArgumentException.class, () -> new ActionType().setName(null));
    }

    @Test
    @DisplayName("Should throw exception when setting empty name")
    void testSetNameWithEmpty() {
        assertThrows(IllegalArgumentException.class, () -> ActionType.builder().name("").build());
        assertThrows(IllegalArgumentException.class, () -> new ActionType().setName(""));
    }

    @Test
    @DisplayName("Should throw exception when setting whitespace-only name")
    ...
    @Test
    @DisplayName("Should throw exception when setting null via setter")
    void testSetNameWithNullViaSetter() {
        ActionType type = new ActionType();
        assertThrows(IllegalArgumentException.class, () -> type.setName(null));
    }
}