...
COMMENT ON COLUMN public.priorities.color_hex_code IS 'Шестнадцатеричный код цвета для визуального выделения приоритета. Например: rgba(255, 0, 0, 1)';
...
COMMENT ON TABLE public.action_types IS 'Хранит все типы действий, происходящие задачами (например, изменение статуса, назначение исполнителя, обновление описания, добавление комментария/вложения).';

COMMENT ON COLUMN public.action_types.id IS 'Уникальный идентификатор типа действия.';
COMMENT ON COLUMN public.action_types.name IS 'Тип действия (например, "STATUS_CHANGE", "ASSIGNED", "COMMENTED").';

COMMENT ON CONSTRAINT uk_action_types_name ON public.action_types IS 'Значение поля `public.action_types.name` должно быть уникальным.';
...
COMMENT ON TABLE public.issue_link_types IS 'Определяет различные виды связей между задачами (например, "blocks" (блокирует), "relates to" (связана), "duplicates" (дублирует)). Позволяет строить граф зависимостей между задачами.';
...
COMMENT ON COLUMN public.issue_link_types.description IS 'Описание связи.';
...
COMMENT ON TABLE public.issue_types IS 'Определяет различные категории задач, которые могут быть созданы в системе (например: Баг, Задача, Пользовательская история, Эпик). Позволяет классифицировать работу.';
...
COMMENT ON COLUMN public.issue_types.description IS 'Описание типа задачи.';
...
COMMENT ON TABLE public.issue_statuses IS 'Определяет различные состояния, через которые может проходить задача в своем жизненном цикле (например: Open, In Progress, Resolved). Статусы являются частью рабочего процесса.';
...
COMMENT ON COLUMN public.issue_statuses.description IS 'Описание статуса.';
...
COMMENT ON TABLE public.workflows IS 'Каждый рабочий процесс описывает жиэненный цикл задачи. Впоследствие использовать ГОСТ 12207 и ГОСТ 57193.';
...
COMMENT ON COLUMN public.workflow_statuses.status_id IS 'Ссылка на статус задачи.';
...
COMMENT ON CONSTRAINT uk_workflow_transitions_unique_pair ON public.workflow_transitions IS 'Значение сочетания полей `public.workflow_transitions.workflow_id`, `public.workflow_transitions.name`, `public.workflow_transitions.from_status_id` и `public.workflow_transitions.to_status_id` должно быть уникальным.';
...
COMMENT ON CONSTRAINT uk_file_types_extension_mime_type ON public.file_types IS 'Значение сочетания полей `public.file_types.extension` и `public.file_types.mime_type` должно быть уникальным.';
...
COMMENT ON COLUMN public.issues.performer_id IS 'Ссылка на пользователя, которому назначена задача.';
...
COMMENT ON COLUMN public.issues.reporter_id IS 'Ссылка на пользователя, который создал задачу.';
...
COMMENT ON COLUMN public.activity_log.user_id IS 'Ссылка на пользователя, который выполнил действие';
COMMENT ON COLUMN public.activity_log.action_type_id IS 'Ссылка на тип действия, которое происходит для задачи';
...
COMMENT ON TABLE public.attachments IS 'Хранит метаданные файлов, прикрепленных к задачам. Сами файлы обычно хранятся в файловой системе или облачном хранилище, в БД — только ссылки на них.';
...
COMMENT ON COLUMN public.attachments.author_id IS 'Ссылка на пользователя, который загрузил файл.';
...
COMMENT ON COLUMN public.attachments.updated_at IS 'Временная метка загрузки файла.';
COMMENT ON COLUMN public.attachments.description IS 'Описание файла.';
...
COMMENT ON CONSTRAINT uk_issue_links_unique_pair ON public.issue_links IS 'Значение сочетания полей `public.issue_links.source_issue_id`, `public.issue_links.target_issue_id` и `public.issue_links.link_type_id` должно быть уникальным.';
...
COMMENT ON TABLE public.issue_watchers IS 'Хранит список пользователей, которые "наблюдают" за определенной задачей и хотят получать уведомления изменений.';
...
COMMENT ON COLUMN public.notification_queue.recipient_id IS 'Ссылка на пользователя-получателя (если известен).';
...
COMMENT ON COLUMN public.notification_statuses.description IS 'Описание статуса обработки уведомления.';

COMMENT ON CONSTRAINT uk_notification_statuses_name_unique ON public.notification_statuses IS 'Значение поля `public.notification_statuses.name` должно быть уникальным.';
...
ALTER TABLE IF EXISTS public.issues
    ADD CONSTRAINT fk_issues_performer FOREIGN KEY (performer_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_issues_performer ON public.issues IS 'Нельзя удалить пользователя, которому назначена задача.';
...
ALTER TABLE IF EXISTS public.issues
    ADD CONSTRAINT fk_issues_workflow FOREIGN KEY (workflow_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_issues_workflow ON public.issues IS 'Нельзя удалить рабочий процесс, если его значение в задаче.';
...
ALTER TABLE IF EXISTS public.projects
    ADD CONSTRAINT fk_projects_default_workflow FOREIGN KEY (default_workflow_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_projects_default_workflow ON public.projects IS 'Нельзя удалить рабочий процесс, если его значение в проекте.';
...
ALTER TABLE IF EXISTS public.workflow_statuses
    ADD CONSTRAINT fk_workflow_statuses_status FOREIGN KEY (status_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_workflow_statuses_status ON public.workflow_statuses IS 'Нельзя удалить статус задачи, если есть связанные статусы рабочего процесса.';
...
ALTER TABLE IF EXISTS public.workflow_statuses
    ADD CONSTRAINT fk_workflow_statuses_workflow FOREIGN KEY (workflow_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_workflow_statuses_workflow ON public.workflow_statuses IS 'Нельзя удалить рабочий процесс, если есть связанные статусы рабочего процесса.';
...
ALTER TABLE IF EXISTS public.workflow_transitions
    ADD CONSTRAINT fk_workflow_transitions_workflow FOREIGN KEY (workflow_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_workflow_transitions_workflow ON public.workflow_transitions IS 'Нельзя удалить рабочий процесс, если есть связанные переходы статуса в рабочем процессе.';
...
ALTER TABLE IF EXISTS public.workflow_transitions
    ADD CONSTRAINT fk_workflow_transitions_from_status FOREIGN KEY (from_status_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_workflow_transitions_from_status ON public.workflow_transitions IS 'Нельзя удалить статус задачи, если есть связанные переходы статуса в рабочем процессе.';
...
ALTER TABLE IF EXISTS public.workflow_transitions
    ADD CONSTRAINT fk_workflow_transitions_to_status FOREIGN KEY (to_status_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_workflow_transitions_to_status ON public.workflow_transitions IS 'Нельзя удалить статус задачи, если есть связанные переходы статуса в рабочем процессе.';
...
ALTER TABLE IF EXISTS public.activity_log
    ADD CONSTRAINT fk_activity_log_action_type FOREIGN KEY (action_type_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_activity_log_action_type ON public.activity_log IS 'Нельзя удалить тип действия, у которого есть осуществленная активность.';
...
ALTER TABLE IF EXISTS public.activity_log
    ADD CONSTRAINT fk_activity_log_user FOREIGN KEY (user_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_activity_log_user ON public.activity_log IS 'Нельзя удалить пользователя, у которого есть осуществленная активность.';
...
ALTER TABLE IF EXISTS public.attachments
    ADD CONSTRAINT fk_attachments_file_type FOREIGN KEY (file_type_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_attachments_file_type ON public.attachments IS 'Нельзя удалить тип файла, если есть связанный документ.';
...
ALTER TABLE IF EXISTS public.issue_watchers
    ADD CONSTRAINT fk_issue_watchers_user FOREIGN KEY (user_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_issue_watchers_user ON public.issue_watchers IS 'Нельзя удалить пользователя, если он является наблюдателем задач.';
...
ALTER TABLE IF EXISTS public.role_permissions
    ADD CONSTRAINT fk_role_permissions_role FOREIGN KEY (role_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_role_permissions_role ON public.role_permissions IS 'Нельзя удалить роль, если есть связанные доступы по роли.';
...
ALTER TABLE IF EXISTS public.role_permissions
    ADD CONSTRAINT fk_role_permissions_permission FOREIGN KEY (permission_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_role_permissions_permission ON public.role_permissions IS 'Нельзя удалить доступ, если есть связанные роли по доступу.';
...
ALTER TABLE IF EXISTS public.project_members
    ADD CONSTRAINT fk_project_members_group FOREIGN KEY (group_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_project_members_group ON public.project_members IS 'Нельзя удалить группу пользователей, если есть связанные члены проекта.';
...
ALTER TABLE IF EXISTS public.project_members
    ADD CONSTRAINT fk_project_members_role FOREIGN KEY (role_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_project_members_role ON public.project_members IS 'Нельзя удалить роль, если есть связанные члены проекта.';
...
ALTER TABLE IF EXISTS public.project_issue_type_workflow_defaults
    ADD CONSTRAINT fk_pitwd_issue_type FOREIGN KEY (issue_type_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_pitwd_issue_type ON public.project_issue_type_workflow_defaults IS 'Нельзя удалить тип задачи, если есть связанные рабочие процессы по умолчанию для проекта/типа задач.';
...
ALTER TABLE IF EXISTS public.user_notification_settings
    ADD CONSTRAINT fk_user_notification_settings_user FOREIGN KEY (user_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_user_notification_settings_user ON public.user_notification_settings IS 'Нельзя удалить пользователя, если есть связанные настройки уведомления пользователя.';
...
ALTER TABLE IF EXISTS public.notification_queue
    ADD CONSTRAINT fk_notification_queue_recipient_user FOREIGN KEY (recipient_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_notification_queue_recipient_user ON public.notification_queue IS 'Нельзя удалить пользователя, являющимся получателем уведомлений.';
...
ALTER TABLE IF EXISTS public.notification_queue
    ADD CONSTRAINT fk_notification_queue_template FOREIGN KEY (template_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE RESTRICT;
COMMENT ON CONSTRAINT fk_notification_queue_template ON public.notification_queue IS 'Нельзя удалить шаблон уведомлений, по которому есть неотправленные уведомления.';
...
-- CREATE INDEX IF NOT EXISTS idx_issues_assignee_id ON public.issues (performer_id);
...
-- CREATE INDEX IF NOT EXISTS idx_attachments_uploader_id ON public.attachments (author_id);
...
-- CREATE INDEX IF NOT EXISTS idx_notification_queue_recipient_user_id ON public.notification_queue (recipient_id);
