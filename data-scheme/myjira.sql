...
COMMENT ON TABLE public.workflows IS 'Каждый рабочий процесс описывает жизненный цикл задачи. Например, один рабочий процесс может быть для багов, другой — для историй. Далее здесь можно указывать процессы ГОСТ Р 527193-2016 и ГОСТ ИСО МЭК 12207 (см. ГОСТ Р ИСО МЭК ТО 15271).';
...
COMMENT ON COLUMN public.activity_log.action_type_id IS 'Ссылка на тип действия, которое происходит с задачей.';
...
COMMENT ON TABLE public.action_types IS 'Хранит все типы действий, происходящие с задачами (например, изменение статуса, назначение исполнителя, обновление описания, добавление комментария/вложения).';
